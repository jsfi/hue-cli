...
module.exports = function(args, config) {
    ...
    if (!hueData.bridge) {
        return helper.errorMsg(`No bridge configured, please set the IP or call discover.`);
    }
    ...
    if (!hueData.username) {
        return helper.errorMsg(`No username configured, please set the username or call createUser.`);
    }
    ...
    return helper.errorMsg(`No action could be recognized.`);
};
