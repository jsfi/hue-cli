...
module.exports = function(args, config) {
    let hueData = require('./helper').loadData(config.data);
    ...
    if (!hueData.bridge) {
        return new Promise((resolve, reject) => {
            reject((`No bridge configured, please set the IP or call discover.`));
        });
    }
    ...
    if (!hueData.username) {
        return new Promise((resolve, reject) => {
            reject((`No username configured, please set the username or call createUser.`));
        });
    }
